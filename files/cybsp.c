...
cy_rslt_t cybsp_init(void)
{
    ...
    initFPU();
        ...
        !defined(CYBSP_SKIP_PPCA_RAM_ENABLE)
    // Ensure PPCA RAM is powered before any early shared-memory access.
    ...
    // Non-secure app and PPCA builds need this explicit call.
    SystemCoreClockUpdate();

    #if ((CY_SYSTEM_CPU_M33) && defined(COMPONENT_NON_SECURE_DEVICE) && \
    defined(CYCFG_MPU_NS_ENABLED))
    ...
    // CYCFG_MPU_NS_ENABLED is generated only when the design configures at least one
    // MPU region, which is also when init_cycfg_protection() is generated.
